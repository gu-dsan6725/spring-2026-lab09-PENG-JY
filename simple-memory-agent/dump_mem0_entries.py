...
"""
    ...
    for user_id in users_to_query:
        ...
        try:
            ...
            memories = response.get("results", []) if isinstance(response, dict) else (response if isinstance(response, list) else [])

            if memories:
                print(f"  Found {len(memories)} memories via user_id (v2)")
                for mem in memories:
                    mem["_queried_user_id"] = user_id
                    all_entries.append(mem)
                print(f"  Total: {len(memories)} memories for {user_id}")
            else:
